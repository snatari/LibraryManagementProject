...
<style>
  body {
        ...
        /* background-image:url("https://www.elsetge.cat/myimg/f/30-308744_bubble-wallpaper-hd-wallpaper-pleasant-background-images-hd.jpg"); */
       /* background-image: url("https://cache.desktopnexus.com/cropped-wallpapers/645/645145-1536x864-[DesktopNexus.com].jpg?st=pjl0KyW__yRwIPouRhqtDw&e=1581187882"); */
        /* background-image:url("https://ivieleaguehomes.com/wp-content/uploads/2019/03/spring-birds.jpg"); */
...
<body>
    <table>
     ...
     <c:forEach items="${dvds}" var="dvd">
     	<tr>
     	<td><img src="${dvd.imgSource}" style="width: 60px; height: 60px;">
     	</td>
     	<td>${dvd.dTitle}</td>
       	<td>${dvd.author}</td>  
     	<td>${dvd.dCopies}</td>
     	<td>${dvd.vRating}</td>
        <td><input type ="checkbox" class ="checks" id ="${dvd.dCopies }"
        value ="${dvd.dTitle}" onclick="CheckOut()"></td>
        <td><input type ="checkbox" class ="holds" id="${dvd.dCopies}"
        value ="${dvd.dTitle}" onclick="Hold()"></td>
        </tr>
        </c:forEach>
    </table>
      <form name="myform" action="dvdCheckout" method="post">
		<div align="center">
			<input type="hidden" id="thisField" name="thisField" value="">
			<input type="hidden" id="anotherField" name="anotherField" value="">
			<input class="tablink" type="Submit" name="dvdCheckout"
