<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Library Events</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
body {
	background-image:
		url("https://image.freepik.com/free-vector/abstract-paper-hexagon-white-background_51543-7.jpg");
	background-repeat: no-repeat;
	background-attachment: fixed;
	background-size: cover;
}

.header {
	background-color: deeppink;
	padding: 30px 16px;
}

.footer {
	position: fixed;
	left: 0;
	bottom: 0;
	width: 100%;
	background-color: gray;
	color: white;
	text-align: center;
}

* {box-sizing: border-box}

/* Set height of body and the document to 100% */
body, html {
	height: 100%;
	margin: 0;
	font-family: Arial;
}

/* Style tab links */
.tablink {
	background-color: black;
	color: white;
	float: left;
	border: none;
	outline: none;
	cursor: pointer;
	padding: 14px 16px;
	font-size: 17px;
	width: 33.333%;
}

.tablink:hover {
	background-color: #777;
}

/* Style the tab content (and add height:100% for full page content) */
.tabcontent {
	color: black;
	display: none;
	padding: 100px 20px;
	height: 100%;
}


/* extra compared to library.jsp */
table {
	font-family: arial, sans-serif;
	border-collapse: collapse;
	width: 100%;
}

td, th {
	border: 1px solid #dddddd;
	text-align: left;
	padding: 8px;
}

tr:nth-child(even) {
	background-color: lightgray;
}
</style>
<script>
function myFunctionEvents()
{
	window.location.href = "Events";
}
function myFunctionItemsDue() {
	window.location.href = "ItemsDue";
}
function myFunctionContact() {
	window.location.href = "Contact";
}
function Checkout() {
	var checks = document.getElementsByClassName('checks');
	var bookschecked = '';
	for (i = 0; i < checks.length; i++) {

		if (checks[i].checked === true) {
			bookschecked += checks[i].value + ",";
		}
	}
	document.getElementById("thisField").value = bookschecked;

}
function Hold() {

	var holds = document.getElementsByClassName('holds');
	var bookshold = '';
	for (i = 0; i < holds.length; i++) {

		if (holds[i].checked === true) {
			bookshold += holds[i].value + ",";
		}
	}

	document.getElementById("anotherField").value = bookshold;
	alert(bookshold);

}



</script>
</head>
<body>
	<header class="header" style="text-align: center">
		<img
			src="https://s3.amazonaws.com/libapps/accounts/50/images/library_logo.jpg"
			style="width: 100px; height: 60px"> <br> <br>Welcome
		To The Library ${username} &nbsp; &nbsp;
		<%
			String type = (String) session.getAttribute("usertype");
			if (type.equals("admin")) {
		%>
		<a href="/Admin"> Admin </a>

		<%
			}
		%>
		&nbsp; &nbsp; <a href="/Logout"> Logout </a> <br> <br>

		<form method="post" action="Search">
			<div class="b" style="text-align: center">
				<label style="color: orange;" style="font-size:small">Type:</label>
				<select name="searchoption">
					<option value="BOOK">book</option>
					<option value="DVD">dvd</option>
					<option value="MAGAZINE">magazine</option>
				</select> <label style="color: yellowgreen;">SearchBy:</label> <select
					name="SearchCategory">
					<option value="ByAuthor">ByAuthor</option>
					<option value="ByTitle">ByTitle</option>
					<option value="ByISBN">ByISBN</option>
				</select> <input type="text" name="">
				<button title="click me">Search</button>
			</div>
		</form>
	</header>
	<button class="tablink" onclick="myFunctionEvents()"> Events </button>
	<button class="tablink" onclick="myFunctionItemsDue()"> Items Due </button>
	<button class="tablink" onclick="myFunctionContact()"> Contact </button>
	

	

	<table>
		<tr>
			<th>Date</th>
			<th>AgeGroup</th>
			<th>EventName</th>
			<th>Duration</th>
			<th>Where</th>
			<!--<th>Register</th>-->
		</tr>
		<tr>
			<td>March 25,2020</td>
			<td>3-5years</td>
			<td>Story Reading</td>
			<td>11:30am to 12:00pm</td>
			<td>2nd floor</td>
			<!--  <td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
		<tr>

			<td>March 27,2020</td>
			<td>1-2years</td>
			<td>PuppetShow</td>
			<td>4:30pm to 5:00pm</td>
			<td>3rd floor</td>
			<!--  <td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
		<tr>

			<td>March 29,2020</td>
			<td>All*</td>
			<td>Family Movie Night</td>
			<td>6pm to 7:30pm</td>
			<td>3rd floor</td>
			<!--  <td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
		<tr>

			<td>March 31,2020</td>
			<td>Teenage</td>
			<td>Math Games</td>
			<td>4:30pm to 5:30pm</td>
			<td>2nd floor</td>
			<!--  <td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
		<tr>
			<td>April 2,2020</td>
			<td>5-8</td>
			<td>Magic Show</td>
			<td>Time:6pmto 6:30pm</td>
			<td>3rd floor</td>
			<!--<td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
		<tr>

			<td>April 9,2020</td>
			<td>6-8years</td>
			<td>Meet the Cartoon Characters</td>
			<td>2:30pm to 3:30pm</td>
			<td>3rd floor</td>
			<!--<td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
		<tr>
			<td>April 13,2020</td>
			<td>12-16</td>
			<td>STAAR test Quiz</td>
			<td>11amto 12:00pm</td>
			<td>3rd floor</td>
			<!--<td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
		<tr>

			<td>April1 9,2020</td>
			<td>18+</td>
			<td>Health and Wellness Day</td>
			<td>1:30pm to 4:30pm</td>
			<td>3rd floor</td>
			<!--<td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
		<tr>

			<td>April 22,2020</td>
			<td>All*</td>
			<td>FAMILY ROBOTICS</td>
			<td>2:30pm to 3:30pm</td>
			<td>3rd floor</td>
			<!--	<td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
		<tr>

			<td>April 25,2020</td>
			<td>5&above</td>
			<td>ORIGAMI NIGHT</td>
			<td>7pm to 8:30pm</td>
			<td>2nd floor</td>
			<!--<td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
		<tr>
			<td>April 30,2020</td>
			<td>All*</td>
			<td>FAMILY DANCE PARTY</td>
			<td>5:00pmto 6:30pm</td>
			<td>3rd floor</td>
			<!--	<td><input type="button" name="Register" value="Register">
</td>-->
		</tr>
	</table>
</body>
</html>